...
const { protect, authorize } = require('../middlewares/authMiddleware'); // Import protect and authorize middlewares
...
router.get('/:id', getAllCategories); // Public route to get a single category by ID
router.put('/update/:id',  upload.single('image'), updateCategory); // Admin only route to update a category
